...
export default function CheckoutPage() {
  ...
  const handlePlaceOrder = async () => {
    ...
    if (!shippingAddress) { toast.error('Delivery address select karein!'); return; }
    ...
    try {
      ...
      toast.error(error?.response?.data?.message || 'Order place nahi hua, dobara try karein!');
    } finally {
      setPlacing(false);
    }
  };
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-3xl shadow-xl p-10 max-w-md w-full text-center">
          ...
          <p className="text-gray-500 text-sm mb-6">Email pe confirmation bhej di gayi hai. Jald hi aapka order process hoga.</p>
          <div className="flex gap-3">
            <button onClick={() => router.push('/orders')} className="flex-1 btn-primary text-white py-3 rounded-xl font-bold text-sm">
              Orders Dekhein
            </button>
            <button onClick={() => router.push('/products')} className="flex-1 border-2 border-gray-200 text-gray-700 py-3 rounded-xl font-bold text-sm hover:border-red-500 hover:text-red-600 transition-colors">
              Shopping
            </button>
          </div>
        </div>
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
              ...
              {addresses.length > 0 && (
                <div className="space-y-3 mb-4">
                  {addresses.map(addr => (
                    <label key={addr.id} className={`flex items-start gap-3 p-4 rounded-xl border-2 cursor-pointer transition-all ${selectedAddress === addr.id && !useCustom ? 'border-red-500 bg-red-50' : 'border-gray-200 hover:border-gray-300'}`}>
                      ...
                      <div>
                        ...
                        {addr.isDefault && <span className="text-xs bg-red-100 text-red-600 font-semibold px-2 py-0.5 rounded-full mt-1 inline-block">Default</span>}
                      </div>
                    </label>
                  ))}
                </div>
              ...
              <label className={`flex items-start gap-3 p-4 rounded-xl border-2 cursor-pointer transition-all ${useCustom ? 'border-red-500 bg-red-50' : 'border-gray-200 hover:border-gray-300'}`}>
                <input type="radio" checked={useCustom} onChange={() => setUseCustom(true)} className="mt-1" />
                <div className="flex-1">
                  <p className="font-semibold text-sm text-gray-900 flex items-center gap-1"><Plus size={14} /> Naya Address</p>
                  {useCustom && (
                    <textarea value={customAddress} onChange={(e) => setCustomAddress(e.target.value)}
                      placeholder="Pura address likhein (ghar number, gali, area, shehar)"
                      rows={3} className="w-full mt-2 border border-gray-200 rounded-xl px-3 py-2 text-sm outline-none focus:border-red-500 resize-none" />
                  )}
                </div>
              </label>
            </div>
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 h-fit">
            ...
            <div className="border-t pt-3 space-y-2 text-sm">
              ...
              <div className="border-t pt-2 flex justify-between font-black text-lg">
                <span>Total</span>
                <span className="text-red-600">{formatPrice((cart?.totalAmount || 0) + (cart && cart.totalAmount >= 2000 ? 0 : 200))}</span>
              </div>
            </div>
            <button onClick={handlePlaceOrder} disabled={placing}
              ...
              {placing ? (
                <><span className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" /> Order Place Ho Raha Hai...</>
              ) : 'Order Place Karein'}
            </button>
          </div>
        </div>
